...
@click.option('--config',
              ...
              default=join(dirname(dirname(abspath(__file__))),'configs/train_bev.yaml'))
...
def main(config, weights, checkpoint, test):
    ...
    if weights is None:
        ...
        model = models.FlowIMG.load_from_checkpoint(weights, hparams=cfg)
        print(model.hparams)
    ...
    checkpoint_saver = ModelCheckpoint(
                                 ...
                                 save_top_k=-1,
                                 every_n_epochs=cfg['train'].get('checkpoint_every_n_epochs', 20)
                                 )
    ...
    if requested_gpus > 1:
        ...
        trainer = Trainer(
            ...
            # denotes hardware (CUDA/CPU) and does not accept ``ddp``.
            strategy='ddp',
            logger=tb_logger,
